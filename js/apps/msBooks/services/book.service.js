...
function addBook(book){
  return new Promise(resolve=>{
    ...
    resolve('book added succesfully!')

  })
}
function localize(googleItems){
  let resBooks=googleItems.map(item=>{
      return{
...
function searchGoogleBook(searchVal){
  return new Promise((resolve, reject)=>{
    var httpRequest = new XMLHttpRequest();
    
    httpRequest.onreadystatechange =  () => {
      console.log( `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchVal}`)
        if (httpRequest.readyState === XMLHttpRequest.DONE && httpRequest.status === 200) {
            var ans = JSON.parse(httpRequest.responseText);
            resolve(ans)
        } else if(httpRequest.readyState === XMLHttpRequest.DONE && httpRequest.status >=300){
          reject( `couldn't find search val on google books`)
        }
    }
    httpRequest.open("GET", `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchVal}`, true);
    httpRequest.send();
  })
}
...
function addReview(bookID, review){
  ...
  return new Promise((resolve, reject)=>{
    ...
    if(!book) {reject('DID NOT REMOVE BOOK')}
    if(!book.reviews){
      book.reviews= new Array() 
    }
    book.reviews.push(review)
    if(!book.rating){
      book.rating={sum:0, dividedBy:0}
    }
    book.rating.sum+=review.rate;
    book.rating.dividedBy++;
    storageService.store(BOOKS_KEY, booksDB);
...
function getNextPrevBookIds(bookID) {
  ...
  var prevIdx = idx - 1;
  if (prevIdx < 0) prevIdx = booksDB.length-1; 
  return {
      prevId: booksDB[prevIdx].id,
      nextId: booksDB[nextIdx].id,
  }
}

function getBookById(ID) {
  return new Promise(resolve => {
    var bookRes = booksDB.find(book => book.id === ID);
    console.log('bookREs in bookservice0', bookRes)
    resolve(bookRes);
  });
}
function removeBookByID(bookID) {
  const idx = booksDB.findIndex(booksDB => booksDB.id === bookID)
  if(idx === -1) return Promise.reject('DID NOT REMOVE BOOK')
  booksDB.splice(idx, 1);
  storageService.store(BOOKS_KEY, booksDB)
  return Promise.resolve('book REMOVED')
}
...
function createBook() {
  ...
  var book = {
    ...
    price: utilService.getRandomInt(5, 250),
    isOnSale: Math.random() > 0.67 ? true : false
  };
  return book;
}
